#!/usr/bin/env python3
"""plot_recall_vs_mix.py — recall vs write fraction, one line per delete model.

Reads the CSV from recall_vs_mix.sh and plots median recall vs write fraction
with min/max error bars (across repeats), one series per model.

    python3 plot_recall_vs_mix.py recall_vs_mix.csv -o recall_vs_mix.png
"""
import argparse
import csv
...
def main():
    ...
    ap.add_argument("csv", help="recall_vs_mix.csv")
    ap.add_argument("-o", "--out", default=None, help="output PNG (default: <csv>.png)")
    ap.add_argument("--title", default="Recall vs write fraction (live-set recall)")
    args = ap.parse_args()
    ...
    # (model, model_name) -> {write_frac: [recall, ...]}
    ...
    with open(args.csv) as f:
        for row in csv.DictReader(f):
            ...
            try:
                series[model][wf].append(float(row["recall"]))
            except (TypeError, ValueError):
                pass

    # nicer labels/colors per model
    ...
    ax.set_ylabel("recall @k (vs brute force over live set)")
